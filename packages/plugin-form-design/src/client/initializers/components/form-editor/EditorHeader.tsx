...
export const EditorHeader = ({ onCancel, schema }) => {
  ...
  const handleSave = async () => {
    ...
    const cardSchema = findSchemaUtils(schema, 'x-decorator', 'FormBlockProvider');
    const newActionBarSchema = findSchemaUtils(cardSchema, 'x-component', 'ActionBar');
...
const MobilePreviewContent = ({ schema }) => {
  ...
  const cardSchema = findSchemaUtils(mobileSchema, 'x-component', 'CardItem') || {};
  cardSchema['x-component-props'] = {
    style: {
      boxShadow: 'none',
    },
  };
  const formSchema = findSchemaUtils(cardSchema, 'x-component', 'FormV2') || {};
