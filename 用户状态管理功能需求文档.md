...
#### Task 2.7: 实现核心方法 5 - initSystemStatuses

**目的:** 初始化系统内置状态

**实现步骤:**
1. 定义三个系统状态 (active, pending, disabled)
2. 检查是否已存在
3. 不存在则插入
 ...
 * 在插件加载时调用,确保基础状态存在
 */
async initSystemStatuses(): Promise<void> {
  try {
    ...
    for (const status of systemStatuses) {
      ...
      if (!existing) {
        ...
        this.logger.info(`Initialized system status: ${status.key}`);
      }
    }

    this.logger.info('System statuses initialization completed');

  } catch (error) {
    this.logger.error('Error initializing system statuses:', error);
    // 不抛出错误,避免影响插件加载
  }
}
```
