...
import { COLLECTION_PASSWORD_POLICY, LOCK_SECONDS, NAMESPACE, WINDOW_SECONDS } from '../../constants';
...
export class PasswordPolicyService {
  ...
  // 缓存前缀
  private readonly CACHE_PREFIX = COLLECTION_PASSWORD_POLICY;
  ...
  async load() {
    ...
    this.app.on('afterStart', async () => {
      // 从配置中读取参数
      const config = await this.db.getRepository(COLLECTION_PASSWORD_POLICY).findOne();
    ...
    this.db.on(`${COLLECTION_PASSWORD_POLICY}.afterSave`, async (model) => {
