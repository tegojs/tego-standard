...
export const SyncRemoteCodeButton: React.FC = () => {
    ...
    async (silent = false) => {
      ...
      try {
        ...
        if (remoteCode) {
          ...
          setIsSynced(true);
          if (!silent) {
            message.success(t('Code synced successfully'));
          }
        } else {
          if (!silent) {
            message.error(t('No code returned from remote source'));
          }
        }
      } catch (error: unknown) {
        // 静默模式下只记录错误，不显示消息
  ...
  const handleSync = async () => {
    ...
    if (codeSource !== 'remote') {
      message.warning(t('Please select remote code source first'));
      return;
    }

    if (!codeType) {
      message.warning(t('Please select code type first'));
      return;
    }

    if (!codeUrl) {
      message.warning(t('Code URL is required for Git'));
      return;
    }
