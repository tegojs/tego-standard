...
describe('tenant plugin collections', () => {
  ...
  it('should restore collection manager tables needed by tenant collection creation', async () => {
    app = await createTenantApp();

    await app.db.sequelize.getQueryInterface().dropTable('collections');

    expect(await app.db.collectionExistsInDb('collections')).toBe(false);

    await ensureTenantBaseTables(app);

    expect(await app.db.collectionExistsInDb('collections')).toBe(true);

    await app.db.getRepository('collections').create({
      values: {
        name: 'tenant_restored_posts',
    ...
    expect(app.db.getCollection('tenant_restored_posts')).toBeTruthy();
  });
