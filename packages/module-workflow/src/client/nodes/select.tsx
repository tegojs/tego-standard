...
export default class extends Instruction {
  title = `{{t("Select record", { ns: "${NAMESPACE}" })}}`;
  type = 'select';
  group = 'collection';
  icon = 'SelectOutlined';
  color = '#a47cef';
  description = `{{t("Select records from a collection. You can use variables from upstream nodes as query conditions.", { ns: "${NAMESPACE}" })}}`;
  fieldset = {
    collection: {
      ...collection,
      'x-reactions': [
    ...
    params: {
      ...
      properties: {
        ...
        summary: {
          type: 'array',
          title: `{{t("Select fields to display in the collection", {  ns: "${NAMESPACE}" })}}`,
          'x-decorator': 'FormItem',
          'x-component': 'CollectionTreeSelect',
          'x-component-props': {
