...
将 `module-tenant` 的 `tenant-filter.ts` 提升为**唯一权威实现**，其他模块全部从 `@tachybase/module-tenant` 导入：
...
**影响范围**：
- `plugin-block-charts`：删除自实现的 `stripTenantFilter` / `applyTenantScope`，改为 `import { applyTenantFilterToContext, stripTenantFilter } from '@tachybase/module-tenant/server'`
- `plugin-action-export`：同上，删除自实现的 worker 级过滤函数
- `module-workflow`：删除 `helpers/tenant-context.ts`，改为从 `module-tenant` 导入

**降级策略**：当 `module-tenant` 未启用时，所有导出函数 no-op（返回原始参数），不引入运行时强制依赖。
...
**建议加固**：在 `DataSourceModel.loadIntoApplication()` 中，紧跟 `setCurrentRole` 之后注册 `setCurrentTenant` 中间件：

```typescript
// data-source.ts loadIntoApplication()
dataSource.resourceManager.use(setCurrentRole, { tag: 'setCurrentRole', before: 'acl', after: 'auth' });
// 新增：在外部数据源管道中也注入租户上下文解析
...
这样外部数据源的 resourceManager 管道自带租户上下文解析，不依赖 `dataSourceManager.use()` 的补偿逻辑。

> 注意：这需要 `setCurrentTenant` 从 `module-tenant` 导入。`module-data-source` 应使用**可选导入**（try-require 或插件存在性检查）避免硬依赖。
...
1. **统一工具函数**：将图表、导出、workflow 三份重复的租户过滤实现替换为 `module-tenant` 的单一导出。
2. **补全遗漏路径**：修复 workflow 的 Create 和手动节点表单。
3. **加固外部数据源守卫链**：在 `loadIntoApplication` 中注入 `setCurrentTenant`。
