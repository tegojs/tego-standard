import {
  EditableSchemaSettingOptions,
  EditableSchemaSettingsItemType,
  EditableSchemaSettingsItemTypeWithoutName,
} from './types';

export class EditableSchemaSettings<T = {}> {
  options: EditableSchemaSettingOptions<T>;
  name: string;
  ...
  constructor(options: EditableSchemaSettingOptions<T>) {
    this.options = Object.assign({ items: [] }, options);
    this.name = options.name;
  }

  add(name: string, item: EditableSchemaSettingsItemTypeWithoutName) {
  ...
  get(nestedName: string): EditableSchemaSettingsItemType | undefined {
