...
describe('formatter', () => {
  ...
  describe('dateFormatFn', () => {
    it('should return correct format for sqlite', () => {
      ...
      const result = dateFormatFn(sequelize, dialect, field, format, '+00:00');
    ...
    it('should return correct format for postgres', () => {
      const sequelize = {
        fn: vi.fn().mockImplementation((fn: string, field: string, reversedTimezone: string, format: string) => ({
          fn,
          format,
          field,
          reversedTimezone,
