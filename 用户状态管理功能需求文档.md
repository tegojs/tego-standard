...
| sort | integer | 排序权重 |
| packageName | string | 定义该状态的插件包名（null 表示系统内置），如 '@tachybase/plugin-password-policy' |
| description | text | 状态描述说明 |
| config | json | 扩展配置项 |

**系统内置状态：**

| key | title | allowLogin | loginErrorMessage | 适用场景 |
|-----|-------|-----------|-------------------|---------|
| active | 正常 | ✅ | - | 正常活跃用户 |
| pending | 待审核 | ❌ | 您的账户正在审核中，请等待管理员审核通过 | 新注册等待审核 |
| disabled | 停用 | ❌ | 您的账户已被停用，如有疑问请联系管理员 | 管理员手动停用 |
...
### 3.3 用户状态历史表 (userStatusHistory) - 新建
...
```
GET /api/userStatusHistory:list?filter={"userId":123}&sort=-createdAt
...
| userStatusHistory:list | 查看状态历史 | admin |
...
- [ ] 创建 `userStatusHistory` 表定义
  - 文件位置：`packages/module-user/src/server/collections/userStatusHistory.ts`
...
#### Task 1.2: 创建数据迁移脚本
- [ ] 创建迁移脚本：添加用户状态相关字段
  - 文件位置：`packages/module-user/src/server/migrations/20250116000000-add-user-status-fields.ts`
  - 功能：添加新字段，设置默认值，为现有用户设置 status='active'
  
- [ ] 创建迁移脚本：初始化系统状态
  - 文件位置：`packages/module-user/src/server/migrations/20250116000001-init-system-statuses.ts`
  - 功能：插入 active, pending, disabled 三个系统内置状态
...
- [ ] 在服务 load 方法中调用 `initSystemStatuses()`
  - 插入 active, pending, disabled 状态
  - 幂等处理（已存在则跳过）

#### Task 4.3: 权限配置
- [ ] 配置状态管理相关权限
  - userStatuses:* 操作权限
  - userStatusHistory:list 查看权限
