...
describe('applyTenantFilter', () => {
  ...
  it('should flatten nested and filters for tenant-scoped reads', () => {
    const mergeParams = vi.fn();
    const ctx = {
      state: {
        currentTenantId: 'tenant-a',
    ...
    expect(mergeParams).toHaveBeenCalledWith({
      filter: {
        $and: [{ record: { id: { $exists: true } } }, { status: 'published' }, { tenantId: 'tenant-a' }],
      },
    });
  });

  it('should preserve or branches while flattening nested and filters', () => {
    const mergeParams = vi.fn();
    const ctx = {
      state: {
        currentTenantId: 'tenant-a',
      ...
      action: {
        ...
        params: {
          filter: {
            $or: [{ $and: [{ status: 'published' }, { active: true }] }, { status: 'draft' }],
          },
        },
        mergeParams,
      },
  ...
  it('should preserve sibling conditions when flattening nested and filters', () => {
    const mergeParams = vi.fn();
    const ctx = {
      state: {
        currentTenantId: 'tenant-a',
    ...
    expect(mergeParams).toHaveBeenCalledWith({
      filter: {
        $and: [{ status: 'published' }, { active: true }, { tenantId: 'tenant-a' }],
      },
    });
  });
