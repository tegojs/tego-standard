...
const EditorFieldProperty = ({ schema, setSchemakey }) => {
  ...
  const fieldSchema = useMemo(() => {
    ...
    return findSchemaUtils(schema, 'x-uid', schemaUID) || null;
  }, [schema, schemaUID]);
...
const FieldPropertiesContent = ({ fieldComponentName }) => {
  ...
  return (
    <div style={{ padding: '10px' }}>
      ...
      <SchemaComponent schema={fullSchema} components={{ ...CMConfigurationComponents, ...itemsComponents }} />
    </div>
  );
};
