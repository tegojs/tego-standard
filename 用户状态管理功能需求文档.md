...
#### Task 3.1: 状态检查中间件
- [x] 创建认证状态检查中间件
  - ✅ 位置：在 UserStatusService 的 `injectLoginCheck()` 方法中
  - ✅ 功能：在登录后检查用户状态
  - ✅ 执行顺序：`after: 'auth'`
...
#### Task 4.1: 注册服务到模块
- [x] 在 `module-user` 的 plugin 类中注册 UserStatusService
  - ✅ 文件位置：`packages/module-user/src/server/server.ts`
  - ✅ 在 load 方法中初始化服务
  - ✅ 暴露到 `app.userStatusService`
  - ✅ 注册中间件和拦截器

#### Task 4.2: ~~初始化系统状态~~ (无需操作)
- ✅ 系统状态由迁移脚本 `20251016170001-init-system-statuses.ts` 自动初始化
- ✅ 无需在代码中手动调用初始化方法
